...
export default function EventInstance({Event} : EventProps)
{
    ...
    return (<>
        <div className="flex flex-row bg-[#272626] h-[20vh] w-[50vw] max-[850px]:h-[22vh] max-[600px]:w-[90vw] rounded-lg items-center" onClick={() => { mainDispatch(SetCurrentEvent(Event)); setClicked(true); }}>
            <div className={"basis-1/6 self-center m-5"}>
                <Image alt={"event-image"} src={Event.Image} height={100} width={130} className={"rounded"}/>
            </div>
            <div className={"flex flex-col self-start mt-2 gap-2 max-[600px]:gap-1 text-sm"}>
